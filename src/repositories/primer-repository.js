...
export const bulkInsertExportData = async (rows) => {
  ...
  // 1. Hitung jumlah record di DB SEBELUM insert
  const totalBefore = await TrLaporanPrimerExport.count();

  // 2. Jalankan bulkCreate (biarkan DB nge-skip yang duplikat)
  await TrLaporanPrimerExport.bulkCreate(rows, {
    ignoreDuplicates: true,
  });

  // 3. Hitung jumlah record di DB SESUDAH insert
  const totalAfter = await TrLaporanPrimerExport.count();

  // 4. Hitung selisihnya yang benar-benar masuk
  const actualInserted = totalAfter - totalBefore; // Pasti ketemu angka 5!
  const actualSkipped = rows.length - actualInserted; // 38 - 5 = 33
